...
function Hotels({ trip }) {
  ...
  return (
    <div>
      <h2 className="font-bold text-2xl my-5 mt-10 text-center text-orange-600">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-5">
        {trip?.tripData?.hotel_options?.map((item, index) => (
          <Link
            ...
            className="text-black hover:text-black"
          >
            <div className="bg-orange-200 p-5 rounded-xl shadow-md h-[500px] hover:scale-95 hover:bg-blue-200 transition-all cursor-pointer">
              <img
                src={photoURLs[item.hotel_name] || "/placeholder.jpg"} // Fallback image if photo URL is not available
